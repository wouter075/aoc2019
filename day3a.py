...
# https://raw.githubusercontent.com/jadenPete/AdventOfCode/master/2019/day3-part1.py
...
def printgrid(g):
    ...
    with open("day3a.txt", "a") as f:
        for x in range(0 - grid, grid):
            line = ""
            for y in range(-1 - grid, grid):
                line = line + g[x][y]
            f.write(str(x).ljust(5) + "| " + line + "\n")


def creategrid(g):
    wires = {}
    for x in range(0 - g, g):
        wires[x] = {}
        for y in range(0 - g, g + 1):
            wires[x][y] = "."
    return wires
...
# create image
data = np.zeros([grid, grid, 3], dtype=np.uint8)
data.fill(255)
